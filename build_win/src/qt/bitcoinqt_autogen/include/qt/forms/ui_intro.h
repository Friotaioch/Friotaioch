...
public:
    ...
    void setupUi(QDialog *Intro)
    {
        ...
        sizeWarningLabel->setObjectName("sizeWarningLabel");
        ...
        lblExplanation3->setObjectName("lblExplanation3");
        lblExplanation3->setWordWrap(true);

        verticalLayout->addWidget(lblExplanation3);

        pruneOptLayout = new QHBoxLayout();
        pruneOptLayout->setObjectName("pruneOptLayout");
        prune = new QCheckBox(Intro);
        prune->setObjectName("prune");

        pruneOptLayout->addWidget(prune);

        pruneGB = new QSpinBox(Intro);
        pruneGB->setObjectName("pruneGB");

        pruneOptLayout->addWidget(pruneGB);

        lblPruneSuffix = new QLabel(Intro);
        lblPruneSuffix->setObjectName("lblPruneSuffix");

        pruneOptLayout->addWidget(lblPruneSuffix);

        horizontalSpacer_2 = new QSpacerItem(40, 20, QSizePolicy::Policy::Expanding, QSizePolicy::Policy::Minimum);

        pruneOptLayout->addItem(horizontalSpacer_2);


        verticalLayout->addLayout(pruneOptLayout);
        ...
        lblPruneSuffix->setBuddy(pruneGB);
    ...
    void retranslateUi(QDialog *Intro)
    {
        ...
        lblExplanation1->setText(QCoreApplication::translate("Intro", "When you click OK, %1 will begin to download and process the full %4 block chain (%2 GB) starting with the earliest transactions in %3 when %4 initially launched.", nullptr));
        lblExplanation2->setText(QCoreApplication::translate("Intro", "This initial synchronisation is very demanding, and may expose hardware problems with your computer that had previously gone unnoticed. Each time you run %1, it will continue downloading where it left off.", nullptr));
        lblExplanation3->setText(QCoreApplication::translate("Intro", "If you have chosen to limit block chain storage (pruning), the historical data must still be downloaded and processed, but will be deleted afterward to keep your disk usage low.", nullptr));
        prune->setText(QCoreApplication::translate("Intro", "Limit block chain storage to", nullptr));
#if QT_CONFIG(tooltip)
        prune->setToolTip(QCoreApplication::translate("Intro", "Reverting this setting requires re-downloading the entire blockchain. It is faster to download the full chain first and prune it later. Disables some advanced features.", nullptr));
#endif // QT_CONFIG(tooltip)
        pruneGB->setSuffix(QCoreApplication::translate("Intro", " GB", nullptr));
    } // retranslateUi
